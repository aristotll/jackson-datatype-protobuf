...
public class MessageDeserializerFactory extends Deserializers.Base {
  ...
  private <T extends Message> ProtobufDeserializer<T, ?> getDeserializer(
    ...
    if (deserializer == null) {
      ProtobufDeserializer<T, ?> newDeserializer = new MessageDeserializer<>(
        messageType,
        config
      );
      ProtobufDeserializer<?, ?> previousDeserializer = deserializerCache.putIfAbsent(
        messageType,
        newDeserializer
      );
      deserializer =
        previousDeserializer == null ? newDeserializer : previousDeserializer;
    }
